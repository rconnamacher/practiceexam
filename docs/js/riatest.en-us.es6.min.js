...
function y(){return new t("string",void 0)};class z extends k{constructor(a){super(a)}}x(z,{s:["question",y()],u:["answer",y()],j:["incorrectAnswers",new v("string",void 0)],H:["notes",y()]});class A extends k{constructor(a){super(a)}}x(A,{g:["questions",new v(z,void 0)]});class B extends f{constructor(){super("div","locale-selector action select")}f(){const a={"en-us":"\ud83c\uddfa\ud83c\uddf8 US English",es:"\ud83c\uddea\ud83c\uddf8 Español"};return`<span aria-hidden="true">${a["en-us"].split(" ")[0]}</span>\n            <select id="lang_selector" aria-hidden="false" aria-label="${"Change Language"}">${Object.keys(a).map((b)=>`<option value="${b}" ${"en-us"===b?"selected":""}>${a[b]}</option>`).join("")}\n            </select>`}i(){const a=this.a("select");a.addEventListener("change",
()=>{const b=a.value;b&&this.ca(b)})}ca(a){location.pathname=location.pathname.replace(/[^/]*$/,"en-us"===a?"":`index-${a}.html`)}};class C extends f{constructor(a){super("section","page homepage");this.b=a}f(){return`\n            <header>\n                <h2>${"Ria Practice Exam"}</h2>\n                <div class="actions"></div>\n            </header>\n            <main>\n                <p><a aria-role="button" class="button button-take-test${this.b.g.length?"":" disabled"}" href="#taketest/5">${"Take Exam"}</a>\n                    ( <input type="number" class="num-questions" value="5"> ${"Questions"})</p>\n                <p><a aria-role="button" class="button" href="#edit">${"Edit Test Bank"}</a></p>\n            </main>\n        `}i(){const a=
this.a("input.num-questions"),b=this.a("a.button-take-test");a.addEventListener("input",()=>{b.href=`#taketest/${a.value}`});const c=this.a("header div.actions");this.v(new B,c)}};let D=1;function E(a){const b=[];for(let c=0,d=a.length;c<d;c++)b.splice(Math.floor(Math.random()*(c+1)),0,a[c]);return b};function F(a){return a.replace(/[&<>"']/g,function(a){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[a]})};class G extends f{constructor(a){super("section","page take-test");this.g=a;this.R=[]}f(){return`\n            <header><h2>${`Sample Test: ${this.g.length} Questions`}</h2></header>\n            <main class="test-questions"></main>\n            <footer>\n                <a class="button button-grade-test">${"Grade Test"}</a>\n                <span class="test-results"></span>\n            </footer>\n        `}i(){this.R=[];const a=this.a("main.test-questions");let b=0;for(let c of this.g){const d=
new H(c,b++);this.v(d,a);this.R.push(d)}this.a("a.button.button-grade-test").addEventListener("click",this.la.bind(this))}la(){let a=0;const b=this.g.length;for(var c of this.R)a+=c.ka()?1:0;c=Math.round(100*a/b);this.a(".test-results").textContent=`${a} correct out of ${b} (${c}%)`}}
